...
def zad1_single_neuron(student_id):
    ...
    class SingleNeuron:
        ...
        def forward(self, x_data):
            ...
            W = self.W
            b = self.b
            return f_act(np.dot(x_data, W) + b)
...
def zad2_two_layer_net(student_id):
    ...
    model.hidden_layer.W[:, 0] = [0.89, -0.58]      # wagi neuronu h1
    model.hidden_layer.W[:, 1] = [-0.8, 0.83]      # wagi neuronu h2
    model.hidden_layer.b[:] = [-0.19, -0.4]         # biasy neuronów h1 i h2
    model.output_layer.W[:, 0] = [-0.23, -0.62]      # wagi neuronu wyjściowego
    model.output_layer.b[:] = [0.11]         # bias neuronu wyjściowego
...
if __name__ == '__main__':
    ...
    student_id = 196691         # Twój numer indeksu, np. 102247
