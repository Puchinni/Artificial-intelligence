...
class Node:
    ...
    def gini_best_score(self, y, possible_splits):
        ...
        for idx in possible_splits:
            left = y[:idx + 1]
            right = y[idx + 1:]
            gini_left = 1 - np.sum((np.unique(left, return_counts=True)[1] / len(left)) ** 2)
            gini_right = 1 - np.sum((np.unique(right, return_counts=True)[1] / len(right)) ** 2)
            gini = len(left) / len(y) * gini_left + len(right) / len(y) * gini_right
            gain = 1 - gini
            if gain > best_gain:
                best_gain = gain
                best_idx = idx
    ...
    def find_best_split(self, X, y, feature_subset):
        ...
        if feature_subset is not None:
            new = np.random.choice(X.shape[1], feature_subset, replace=False)
        else:
            new = range(X.shape[1])
